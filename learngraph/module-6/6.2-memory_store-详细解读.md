...
|---------|---------------------|------------------|---------|
| **Store** | LangGraph 中用于跨线程（会话）存储和检索信息的持久化键值存储系统，支持命名空间分层组织 | 实现 BaseStore 接口的存储后端，如 InMemoryStore（内存）或 PostgresStore（数据库） | ⭐⭐⭐⭐⭐ |
| **InMemoryStore** | 在内存中保存数据的 Store 实现，适合开发和测试，数据在进程重启后丢失 | Python 类，继承自 BaseStore，使用内存字典存储数据 | ⭐⭐⭐⭐ |
| **Namespace（命名空间）** | 用元组表示的分层结构，类似文件系统目录，用于组织和隔离不同用户或类型的数据 | Python 元组类型，如 `("user_1", "memories")`，作为数据分组的键 | ⭐⭐⭐⭐⭐ |
| **Key（键）** | 在命名空间内标识特定对象的字符串，可以是固定名称或 UUID | Python 字符串类型，如 `"user_memory"` 或 `str(uuid.uuid4())` | ⭐⭐⭐⭐⭐ |
| **Value（值）** | Store 中存储的实际数据，必须是 Python 字典类型，可包含任意可序列化的数据 | Python dict 类型，如 `{"memory": "User's name is Lance"}` | ⭐⭐⭐⭐⭐ |
| **put() 方法** | Store 的保存操作，将数据写入指定的命名空间和键，如果键已存在则覆盖 | 方法签名：`put(namespace: tuple, key: str, value: dict) -> None` | ⭐⭐⭐⭐⭐ |
| **get() 方法** | Store 的精确获取操作，通过命名空间和键获取特定的单个对象 | 方法签名：`get(namespace: tuple, key: str) -> Item`，返回 Item 对象或 None | ⭐⭐⭐⭐⭐ |
| **search() 方法** | Store 的搜索操作，返回指定命名空间下的所有对象列表 | 方法签名：`search(namespace: tuple) -> list[Item]`，遍历该命名空间 | ⭐⭐⭐⭐⭐ |
| **短期记忆（Within-Thread）** | 使用 MemorySaver (Checkpointer) 实现的单次会话记忆，保存对话历史和状态 | 通过 thread_id 标识，存储在 checkpoint 中，会话期间有效 | ⭐⭐⭐⭐⭐ |
| **长期记忆（Across-Thread）** | 使用 InMemoryStore (Store) 实现的跨会话记忆，持久化保存用户信息 | 通过 namespace + key 标识，跨所有会话共享和访问 | ⭐⭐⭐⭐⭐ |
| **语义记忆（Semantic Memory）** | 关于事实和知识的记忆，如用户姓名、位置、兴趣等结构化信息 | 存储在 Store 中的字典数据，如 `{"user_name": "Lance", "interests": ["biking"]}` | ⭐⭐⭐⭐ |
| **热路径（Hot Path）** | 在用户等待响应的过程中执行的操作，如实时保存记忆（同步处理） | 在主执行流程中调用的函数，会增加用户感知的延迟 | ⭐⭐⭐ |
