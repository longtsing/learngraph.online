...
|---------|---------------------|------------------|---------|
| **Breakpoint** | 图执行过程中的主动暂停点，可在节点执行前/后中断，等待人工干预后继续执行 | N/A (LangGraph 特有概念) | ⭐⭐⭐⭐⭐ |
| **interrupt_before** | 编译时设置的断点参数，在指定节点执行前暂停图的运行 | `graph.compile(interrupt_before=["node"])` 的关键字参数 | ⭐⭐⭐⭐⭐ |
| **interrupt_after** | 编译时设置的断点参数，在指定节点执行后暂停图的运行 | `graph.compile(interrupt_after=["node"])` 的关键字参数 | ⭐⭐⭐⭐ |
| **Checkpointer** | 状态持久化管理器，自动保存图执行过程中的状态快照，支持断点恢复和状态回溯 | 实现检查点协议的类，如 `MemorySaver()` | ⭐⭐⭐⭐⭐ |
| **MemorySaver** | 内存中的 checkpointer 实现，适用于开发测试，进程重启后数据丢失 | `from langgraph.checkpoint.memory import MemorySaver` | ⭐⭐⭐⭐ |
| **get_state()** | 获取图当前状态的方法，返回 StateSnapshot 对象，包含状态值、下一个节点等信息 | `state = graph.get_state(config)` | ⭐⭐⭐⭐⭐ |
| **StateSnapshot** | 状态快照对象，包含 values(状态数据)、next(下一节点)、tasks(待执行任务)等属性 | TypedDict 或类似结构，包含多个属性字段 | ⭐⭐⭐⭐ |
| **stream(None)** | 从断点恢复执行的关键用法，第一个参数传入 None 表示不添加新输入，使用当前状态继续 | `for event in graph.stream(None, config)` | ⭐⭐⭐⭐⭐ |
| **Thread** | 线程配置，用于标识和隔离不同的会话，通过 thread_id 区分不同用户或对话 | `config = {"configurable": {"thread_id": "1"}}` 字典结构 | ⭐⭐⭐⭐⭐ |
| **tools_condition** | LangGraph 预定义的条件函数，判断 AI 消息是否包含工具调用，自动路由到工具节点或结束 | `from langgraph.prebuilt import tools_condition` | ⭐⭐⭐⭐ |
| **ToolNode** | LangGraph 预定义的工具执行节点，自动执行 AI 消息中的工具调用并返回结果 | `from langgraph.prebuilt import ToolNode` | ⭐⭐⭐⭐ |
| **Human-in-the-Loop** | 人机协同模式，包括审批(Approval)、调试(Debugging)、编辑(Editing)三大应用场景 | N/A (设计模式概念) | ⭐⭐⭐⭐⭐ |
...
以下是一个完整的、可以直接在 Jupyter Notebook 中运行的代码示例，演示 Breakpoints 的核心功能：

```python
# ============================================================
# LangGraph Breakpoints 完整示例
# 演示：工具调用审批（Human-in-the-Loop）
# ============================================================
...
builder.add_conditional_edges(
    "assistant",
    tools_condition,  # 如果有工具调用 → tools 节点；否则 → END
)
builder.add_edge("tools", "assistant")

# 编译图：关键参数 interrupt_before=["tools"]
memory = MemorySaver()
graph = builder.compile(
    interrupt_before=["tools"],  # 在 tools 节点执行前中断
    checkpointer=memory          # 需要 checkpointer 支持状态持久化
)
...
# --------------------------
# 7. 第一次执行：触发 Breakpoint
# --------------------------
print("\n" + "=" * 60)
print("🚀 第一次执行：发送计算请求")
print("=" * 60)

initial_input = {"messages": [HumanMessage(content="Multiply 7 and 8")]}
...
print(f"当前消息数量: {len(state.values['messages'])}")
...
# --------------------------
print("\n" + "=" * 60)
print("🔄 完整交互式审批流程（新的 thread）")
print("=" * 60)

# 使用新的 thread
thread2 = {"configurable": {"thread_id": "breakpoint-demo-2"}}
input2 = {"messages": [HumanMessage(content="Add 100 and 200, then divide by 3")]}

# 执行直到 breakpoint
for event in graph.stream(input2, thread2, stream_mode="values"):
    event["messages"][-1].pretty_print()

# 检查是否有待审批的工具调用
state2 = graph.get_state(thread2)
while state2.next:  # 只要有下一个节点（即在 breakpoint 处）
    print(f"\n🛑 Breakpoint: 即将执行 {state2.next}")

    # 显示工具调用信息
    last_msg = state2.values["messages"][-1]
    if isinstance(last_msg, AIMessage) and last_msg.tool_calls:
        for tc in last_msg.tool_calls:
            print(f"   待执行: {tc['name']}({tc['args']})")

    # 模拟用户批准（实际应用中可以使用 input() 获取用户输入）
    print("   ➡️ 自动批准，继续执行...")
...
```
📊 图结构可视化：
[显示 Mermaid 图：START → assistant → tools → assistant → END]
...
| 要点 | 说明 |
|------|------|
| `interrupt_before=["tools"]` | 在 tools 节点执行前暂停，等待人工审批 |
| `checkpointer=memory` | 必需！用于保存断点状态，支持后续恢复 |
| `graph.stream(None, thread)` | 使用 `None` 作为输入，从上次断点继续执行 |
| `graph.get_state(thread)` | 获取当前状态，检查 `.next` 属性判断是否在断点处 |
| `state.values["messages"][-1].tool_calls` | 获取待执行的工具调用信息，用于向用户展示 |
