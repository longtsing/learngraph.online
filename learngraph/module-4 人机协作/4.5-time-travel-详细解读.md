...
|---------|---------------------|------------------|---------|
| **Time Travel** | 图历史状态的浏览、回放和分叉能力，允许从任意历史点重新执行或创建新分支 | 基于 checkpointer 保存的状态快照实现 | ⭐⭐⭐⭐⭐ |
| **get_state_history()** | 获取图完整执行历史的方法，返回按时间倒序排列的所有 StateSnapshot | `list(graph.get_state_history(config))` | ⭐⭐⭐⭐⭐ |
| **StateSnapshot** | 状态快照对象，包含 values(状态数据)、config(含 checkpoint_id)、metadata 等 | LangGraph 返回的状态快照数据结构 | ⭐⭐⭐⭐⭐ |
| **Checkpoint** | 图执行过程中某个时刻的完整状态备份，由 checkpointer 自动在每个节点后保存 | 持久化的状态快照，包含唯一的 checkpoint_id | ⭐⭐⭐⭐⭐ |
| **checkpoint_id** | 检查点的唯一标识符，用于指定回放或分叉的起点 | 字符串类型，通常是 UUID 格式 | ⭐⭐⭐⭐⭐ |
| **回放(Replaying)** | 从历史 checkpoint 重新执行图，不修改状态，用于重现执行过程 | `graph.stream(None, checkpoint_config)` | ⭐⭐⭐⭐⭐ |
| **分叉(Forking)** | 从历史 checkpoint 创建新执行分支，修改状态后继续执行，探索替代路径 | update_state() + stream(None) 组合 | ⭐⭐⭐⭐⭐ |
| **parent_config** | StateSnapshot 的属性，指向父 checkpoint 的配置，用于追溯历史链 | 字典类型，包含父 checkpoint_id | ⭐⭐⭐⭐ |
| **消息覆盖机制** | 分叉时保持相同的 Message ID 实现消息替换，而非追加新消息 | 利用 add_messages reducer 的 ID 匹配逻辑 | ⭐⭐⭐⭐⭐ |
| **历史分支** | 分叉操作创建的新执行路径，与原始历史并存，互不影响 | 类似 Git 的分支概念 | ⭐⭐⭐⭐ |
| **Thread 隔离** | 不同 thread_id 拥有独立的状态和历史，互不干扰 | 通过 configurable.thread_id 区分 | ⭐⭐⭐⭐⭐ |
| **MemorySaver** | 内存版 checkpointer，适合开发测试，数据不持久化 | `from langgraph.checkpoint.memory import MemorySaver` | ⭐⭐⭐⭐ |
...
![Time Travel History Visualization](https://cdn.prod.website-files.com/65b8cd72835ceeacd4449a53/66dbb038211b544898570be3_time-travel1.png)
...
![Replaying from Checkpoint](https://cdn.prod.website-files.com/65b8cd72835ceeacd4449a53/66dbb038a0bd34b541c78fb8_time-travel2.png)
...
![Forking from Checkpoint](https://cdn.prod.website-files.com/65b8cd72835ceeacd4449a53/66dbb038f89f2d847ee5c336_time-travel3.png)
...
以下是一个完整的、可以直接在 Jupyter Notebook 中运行的代码示例，演示 Time Travel 的三大功能：
...
# 关键：必须有 checkpointer 才能使用 Time Travel
memory = MemorySaver()
graph = builder.compile(checkpointer=memory)
...
thread = {"configurable": {"thread_id": "time-travel-demo"}}
initial_input = {"messages": [HumanMessage(content="Multiply 2 and 3")]}
...
all_states = list(graph.get_state_history(thread))
print(f"\n共有 {len(all_states)} 个历史状态（checkpoints）:")
...
for i, state in enumerate(all_states):
    ...
    print(f"  [{i}] Step {step}: → {next_node} | 消息数: {msg_count} | ID: {checkpoint_id}...")
...
# 查看用户输入后、assistant 执行前的状态
to_inspect = all_states[-2]  # 倒数第二个是初始输入后的状态
print(f"\n选择的历史状态 (Step {to_inspect.metadata.get('step', 'N/A')}):")
print(f"  下一个节点: {to_inspect.next}")
print(f"  消息内容: {to_inspect.values['messages'][0].content}")
print(f"  Checkpoint ID: {to_inspect.config['configurable']['checkpoint_id'][:20]}...")
...
to_replay = all_states[-2]
print(f"\n从 Step {to_replay.metadata.get('step', 'N/A')} 回放:")
print("(注意: 回放不修改状态，只是重新执行)")
...
original_msg = to_fork.values["messages"][0]
print(f"\n原始消息: {original_msg.content}")
print(f"原始消息 ID: {original_msg.id}")

# 关键：保持相同的 ID 以覆盖消息
new_input = HumanMessage(
    content="Multiply 5 and 5",
    id=original_msg.id  # 保持相同 ID！
)

print(f"\n修改为: {new_input.content}")
...
print(f"\n新的 Checkpoint ID: {fork_config['configurable']['checkpoint_id'][:20]}...")
...
print(f"\n当前最新消息:")
print(f"  {current_state.values['messages'][-1].content}")

# 检查历史是否包含分叉
new_history = list(graph.get_state_history(thread))
print(f"\n历史状态数: {len(new_history)} (原: {len(all_states)})")
...
# 找到原始执行的最终结果
original_final = all_states[0]  # 原始执行的最终状态
print(f"\n原始执行:")
print(f"  输入: Multiply 2 and 3")
for msg in original_final.values["messages"]:
...
# 分叉后的结果
print(f"\n分叉执行:")
print(f"  输入: Multiply 5 and 5")
print(f"  结果: {current_state.values['messages'][-1].content}")
...
base_msg_id = base_state.values["messages"][0].id

branches = [
    ("3 * 3", "Multiply 3 and 3"),
    ("7 * 7", "Multiply 7 and 7"),
    ("10 * 10", "Multiply 10 and 10"),
]
...
for name, content in branches:
    ...
    fork = graph.update_state(
        base_state.config,
        {"messages": [HumanMessage(content=content, id=base_msg_id)]}
    )

    # 执行并收集结果
    final_msg = None
    for event in graph.stream(None, fork, stream_mode="values"):
        final_msg = event["messages"][-1]

    if final_msg and isinstance(final_msg, AIMessage):
        results[name] = final_msg.content
...
```
📊 图结构可视化：
[显示图：START → assistant → tools → assistant → END]
...
共有 5 个历史状态（checkpoints）:
  [0] Step 4: → END | 消息数: 4 | ID: abc12345...
  [1] Step 3: → assistant | 消息数: 3 | ID: def67890...
  [2] Step 2: → tools | 消息数: 2 | ID: ghi11111...
  [3] Step 1: → assistant | 消息数: 1 | ID: jkl22222...
  [4] Step 0: → assistant | 消息数: 1 | ID: mno33333...

============================================================
🌿 分叉历史状态
============================================================

原始消息: Multiply 2 and 3
修改为: Multiply 5 and 5
...
| 要点 | 说明 |
|------|------|
| `checkpointer=MemorySaver()` | 必需！Time Travel 依赖 checkpointer 保存历史 |
| `graph.get_state_history(thread)` | 获取完整历史（按时间倒序） |
| `graph.stream(None, checkpoint.config)` | 从指定 checkpoint 回放或继续执行 |
| `graph.update_state(config, updates)` | 修改历史状态创建分叉 |
| 保持 Message ID | 分叉时使用相同 ID 覆盖消息，而非追加 |

**Time Travel 三大功能对比：**

| 功能 | 操作 | 结果 |
|------|------|------|
| 浏览历史 | `get_state_history(thread)` | 查看所有 checkpoint |
| 回放 | `stream(None, checkpoint.config)` | 重新执行（不修改） |
| 分叉 | `update_state()` + `stream()` | 修改状态并执行 |
...
![Flow Diagram](images/time-travel-output-6-0.jpg)
