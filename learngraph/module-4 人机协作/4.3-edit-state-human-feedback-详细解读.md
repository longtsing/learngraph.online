...
|---------|---------------------|------------------|---------|
| **状态编辑(State Editing)** | 在图执行过程中暂停并修改状态数据的能力，允许人类干预 AI 决策路径 | 通过 update_state() API 实现 | ⭐⭐⭐⭐⭐ |
| **update_state()** | 更新图状态的核心方法，支持修改状态值并指定更新来源节点(as_node 参数) | `graph.update_state(config, values, as_node=...)` | ⭐⭐⭐⭐⭐ |
| **as_node 参数** | update_state 的关键参数，指定状态更新来自哪个节点，决定后续执行流程 | 字符串类型，值为节点名称 | ⭐⭐⭐⭐⭐ |
| **Reducer 机制** | 状态字段的合并策略，如 add_messages 会根据消息 ID 决定追加或覆盖 | Annotated 类型注解实现 | ⭐⭐⭐⭐⭐ |
| **add_messages** | MessagesState 的内置 reducer，有 ID 则覆盖同 ID 消息，无 ID 则追加新消息 | `Annotated[list, add_messages]` | ⭐⭐⭐⭐⭐ |
| **Message ID** | 消息的唯一标识符，用于消息去重和覆盖，状态编辑时保持 ID 实现消息替换 | BaseMessage 的 id 属性(字符串或 UUID) | ⭐⭐⭐⭐⭐ |
| **Human Feedback Node** | 专门用于接收人类输入的空操作节点，作为人机交互的断点位置 | 返回值为 None 或 pass 的节点函数 | ⭐⭐⭐⭐ |
| **MessagesState** | LangGraph 预定义的状态类，包含 messages 字段和 add_messages reducer | `from langgraph.graph import MessagesState` | ⭐⭐⭐⭐⭐ |
| **RunnableConfig** | LangChain 的配置对象，传递运行时参数(如 callbacks、streaming 配置) | `from langchain_core.runnables import RunnableConfig` | ⭐⭐⭐ |
| **RemoveMessage** | 特殊消息类型，用于从状态中删除指定 ID 的消息(通过 add_messages 处理) | `RemoveMessage(id=message_id)` | ⭐⭐⭐⭐ |
| **覆盖 vs 追加** | 状态更新的两种模式:保持 ID 覆盖原消息，不提供 ID 追加新消息 | 由 add_messages reducer 的逻辑决定 | ⭐⭐⭐⭐⭐ |
...
以下是一个完整的、可以直接在 Jupyter Notebook 中运行的代码示例，演示状态编辑和人工反馈的核心功能：

```python
# ============================================================
# LangGraph 状态编辑与人工反馈 完整示例
# 演示：update_state、as_node、消息覆盖与追加
# ============================================================
...
builder.add_edge("human_feedback", "assistant")
builder.add_conditional_edges("assistant", tools_condition)
builder.add_edge("tools", "human_feedback")  # 工具执行后回到人工反馈节点
...
# --------------------------
print("\n" + "=" * 60)
print("📘 演示 1：追加消息（不提供消息 ID）")
print("=" * 60)

thread1 = {"configurable": {"thread_id": "demo-append"}}
initial_input = {"messages": [HumanMessage(content="Multiply 2 and 3")]}

# 执行到断点
print("\n🚀 执行到断点...")
for event in graph.stream(initial_input, thread1, stream_mode="values"):
...
print(f"\n📍 当前暂停位置: {state1.next}")
print(f"📨 消息数量: {len(state1.values['messages'])}")

# 追加新消息（不提供 ID）
print("\n✏️ 追加新消息...")
graph.update_state(
    thread1,
    {"messages": [HumanMessage(content="Actually, multiply 3 and 3 instead!")]},
    as_node="human_feedback"  # 模拟 human_feedback 节点的输出
)

# 验证追加结果
state1_updated = graph.get_state(thread1)
print(f"📨 更新后消息数量: {len(state1_updated.values['messages'])}")
print("\n所有消息:")
for i, msg in enumerate(state1_updated.values['messages']):
    print(f"  {i+1}. [{msg.__class__.__name__}] {msg.content[:50]}...")

# 继续执行
print("\n▶️ 继续执行...")
for event in graph.stream(None, thread1, stream_mode="values"):
    if event.get("messages"):
        event["messages"][-1].pretty_print()

# --------------------------
# 7. 演示 2：消息覆盖（提供相同的消息 ID）
# --------------------------
print("\n" + "=" * 60)
print("📗 演示 2：覆盖消息（保留消息 ID）")
print("=" * 60)

thread2 = {"configurable": {"thread_id": "demo-overwrite"}}
initial_input2 = {"messages": [HumanMessage(content="Add 5 and 5")]}
...
last_message = state2.values['messages'][-1]
original_id = last_message.id
print(f"\n📌 原始消息 ID: {original_id}")
print(f"📌 原始内容: {last_message.content}")

# 创建新消息，保留相同的 ID 以覆盖
modified_message = HumanMessage(
    content="Add 10 and 10",
    id=original_id  # 关键：保留原 ID
)
...
# 验证覆盖结果
state2_updated = graph.get_state(thread2)
print(f"📨 更新后消息数量: {len(state2_updated.values['messages'])} (应该仍是 1)")
print(f"📌 更新后内容: {state2_updated.values['messages'][-1].content}")
...
# --------------------------
# 8. 演示 3：检查 as_node 参数的作用
# --------------------------
print("\n" + "=" * 60)
print("📕 演示 3：as_node 参数的作用")
print("=" * 60)

thread3 = {"configurable": {"thread_id": "demo-as-node"}}
initial_input3 = {"messages": [HumanMessage(content="Multiply 4 and 4")]}

# 执行到断点
for event in graph.stream(initial_input3, thread3, stream_mode="values"):
    pass

state3_before = graph.get_state(thread3)
print(f"暂停位置: {state3_before.next}")

# 使用 as_node 更新状态
graph.update_state(
    thread3,
    {"messages": [HumanMessage(content="Good, proceed with 4 x 4")]},
    as_node="human_feedback"  # 指定为 human_feedback 节点的输出
)

state3_after = graph.get_state(thread3)
print(f"更新后，下一个节点: {state3_after.next}")
print("解释: as_node='human_feedback' 告诉图这是 human_feedback 的输出")
print("      因此图会继续执行 human_feedback 之后的节点（assistant）")
...
thread4 = {"configurable": {"thread_id": "demo-full-loop"}}
initial_input4 = {"messages": [HumanMessage(content="What is 7 times 8?")]}

# 执行直到完成
iteration = 0
current_input = initial_input4
...
while True:
    ...
    # 执行到断点
    for event in graph.stream(current_input, thread4, stream_mode="values"):
        if event.get("messages"):
            last_msg = event["messages"][-1]
            # 只打印新消息
            if isinstance(last_msg, AIMessage):
                print(f"🤖 AI: {last_msg.content[:100] if last_msg.content else '[工具调用]'}")
                if last_msg.tool_calls:
                    for tc in last_msg.tool_calls:
                        print(f"   📞 调用工具: {tc['name']}({tc['args']})")

    # 检查是否完成
    state4 = graph.get_state(thread4)
    if not state4.next:
        print("\n✅ 执行完成!")
        break

    # 模拟用户确认（自动）
    print(f"⏸️ 暂停于: {state4.next}")
    print("   [自动确认继续]")
...
# 显示最终结果
final_state = graph.get_state(thread4)
print(f"\n📊 最终对话（共 {len(final_state.values['messages'])} 条消息）:")
...
```
📊 图结构可视化：
[显示图：START → human_feedback → assistant → tools → human_feedback... → END]
...
所有消息:
  1. [HumanMessage] Multiply 2 and 3...
  2. [HumanMessage] Actually, multiply 3 and 3 instead!...
...
| 要点 | 说明 |
|------|------|
| `human_feedback` 节点 | 空操作节点，仅作为暂停点和人机交互入口 |
| `interrupt_before=["human_feedback"]` | 在 human_feedback 前暂停，等待用户输入 |
| `as_node="human_feedback"` | 更新状态后，告诉图这是 human_feedback 的输出 |
| 无 ID → 追加 | `HumanMessage(content="...")` 会追加新消息 |
| 有 ID → 覆盖 | `HumanMessage(content="...", id=orig_id)` 会覆盖同 ID 消息 |
| `graph.stream(None, thread)` | 从当前状态继续执行，不添加新输入 |
