...
|---------|---------------------|------------------|---------|
| **Human-in-the-Loop** | 人机协同设计模式，在 AI 执行关键操作前引入人类判断和反馈 | N/A (架构设计模式) | ⭐⭐⭐⭐⭐ |
| **Breakpoint** | 图执行过程中的主动暂停点，支持在节点前/后中断等待人工干预 | `interrupt_before/after` 参数实现 | ⭐⭐⭐⭐⭐ |
| **interrupt_before** | 编译时设置的静态断点，在指定节点执行前暂停 | `graph.compile(interrupt_before=["node"])` | ⭐⭐⭐⭐⭐ |
| **NodeInterrupt** | 动态断点核心类，在节点内抛出此异常触发条件性中断 | `raise NodeInterrupt("原因")` | ⭐⭐⭐⭐⭐ |
| **Checkpointer** | 状态持久化管理器，自动保存执行历史，支持断点恢复和时间旅行 | `MemorySaver()` 或 `PostgresSaver()` 等 | ⭐⭐⭐⭐⭐ |
| **get_state()** | 获取图当前状态的方法，返回 StateSnapshot 对象 | `state = graph.get_state(config)` | ⭐⭐⭐⭐⭐ |
| **stream(None)** | 从断点恢复执行的关键用法，None 表示使用当前状态继续 | `graph.stream(None, config)` | ⭐⭐⭐⭐⭐ |
| **update_state()** | 修改图状态的方法，支持指定更新来源节点(as_node参数) | `graph.update_state(config, values, as_node=...)` | ⭐⭐⭐⭐⭐ |
| **as_node 参数** | 指定状态更新来自哪个节点，决定后续执行流程 | update_state 的关键字参数 | ⭐⭐⭐⭐⭐ |
| **Message ID** | 消息唯一标识符，用于消息去重和覆盖，状态编辑核心机制 | BaseMessage 的 id 属性 | ⭐⭐⭐⭐⭐ |
| **add_messages** | MessagesState 的 reducer，根据 ID 决定追加或覆盖消息 | `Annotated[list, add_messages]` | ⭐⭐⭐⭐⭐ |
| **Streaming** | 图执行过程中实时输出结果的技术，支持多种粒度的流式输出 | 生成器或异步生成器实现 | ⭐⭐⭐⭐⭐ |
| **stream_mode** | 控制流式输出粒度的参数(updates/values/messages) | `graph.stream(..., stream_mode="values")` | ⭐⭐⭐⭐⭐ |
| **astream_events** | 异步流式事件 API，支持 Token 级流式输出 | `async for event in graph.astream_events(..., version="v2")` | ⭐⭐⭐⭐⭐ |
| **Time Travel** | 浏览、回放和分叉历史状态的能力，用于调试和测试 | 基于 checkpointer 的历史管理 | ⭐⭐⭐⭐⭐ |
| **get_state_history()** | 获取完整执行历史的方法，返回所有 StateSnapshot | `list(graph.get_state_history(config))` | ⭐⭐⭐⭐⭐ |
| **回放(Replaying)** | 从历史 checkpoint 重新执行图，不修改状态 | `graph.stream(None, checkpoint_config)` | ⭐⭐⭐⭐ |
| **分叉(Forking)** | 从历史状态创建新执行分支，修改状态后探索替代路径 | update_state() + stream(None) 组合 | ⭐⭐⭐⭐⭐ |
| **Thread** | 线程配置，用于标识和隔离不同会话，通过 thread_id 区分 | `{"configurable": {"thread_id": "1"}}` | ⭐⭐⭐⭐⭐ |
| **StateSnapshot** | 状态快照对象，包含 values、next、config、metadata 等属性 | LangGraph 返回的数据结构 | ⭐⭐⭐⭐ |
...
```mermaid
graph TB
    A[Human-in-the-Loop 体系] --> B[Breakpoints]
