# 10.8.1 TradingAgent 数据流中间结果回放

本节按 TradingAgent 的数据流顺序逐步回放：每到一个节点，说明节点职责、读取/写入的状态，再直接贴出该节点的原始输出（保持 JSON 中的 Markdown 格式，无二次整理）。原始数据文件：`learngraph/module-10 完整案例-TradingAgent/extracted_reports.json`。

## 数据流顺序概览
- 状态载体按照 LangGraph 节点顺序逐步写入：`market_report → sentiment_report → news_report → fundamentals_report → bull_history → bear_history → judge_decision → investment_plan → trader_investment_plan → risky_history → safe_history → neutral_history → risk_judge_decision → final_trade_decision`。
- 各节点的原始输出均来自 `extracted_reports.json` 对应字段，以下全部为未经重新排版的原文。

## 1. 市场技术分析（market_report）
- 功能：生成 NVDA 的技术面报告，写入 `state["market_report"]`。
- 上游：行情数据；无依赖前置节点。
- 下游：为情绪、新闻、基础面及辩论环节提供价格与指标依据。
<details>
<summary>原始输出（保持原样）</summary>
...
## 2. 情绪分析（sentiment_report）
- 功能：梳理社交/新闻情绪走势，写入 `state["sentiment_report"]`。
- 上游：外部情绪数据；与行情并行产生。
- 下游：供看涨/看跌分析引用情绪温度。
<details>
<summary>原始输出（保持原样）</summary>
...
## 3. 新闻分析（news_report）
- 功能：汇总一周宏观与 NVDA 相关新闻，写入 `state["news_report"]`。
- 上游：新闻抓取结果。
- 下游：作为情景背景输入辩论。
<details>
<summary>原始输出（保持原样）</summary>
...
## 4. 基础面分析（fundamentals_report）
- 功能：输出财报指标与估值概览，写入 `state["fundamentals_report"]`。
- 上游：财务/估值数据。
- 下游：被后续多方观点引用。
<details>
<summary>原始输出（保持原样）</summary>
...
## 5. 看涨分析师发言（bull_history）
- 功能：基于前四个报告给出看涨论据，写入 `state["bull_history"]`。
- 上游：market/sentiment/news/fundamentals 报告。
- 下游：进入与看跌观点的首轮辩论。
<details>
<summary>原始输出（保持原样）</summary>
...
## 6. 看跌分析师发言（bear_history）
- 功能：对同一信息集给出风险视角，写入 `state["bear_history"]`。
- 上游：同上。
- 下游：供裁判综合。
<details>
<summary>原始输出（保持原样）</summary>
...
## 7. 首轮裁判决策（judge_decision）
- 功能：综合 bull/bear 输出初步建议，写入 `state["judge_decision"]`。
- 上游：bull_history, bear_history。
- 下游：为投资计划提供方向。
<details>
<summary>原始输出（保持原样）</summary>
...
## 8. 投资计划草案（investment_plan）
- 功能：把裁判结论转成可执行计划，写入 `state["investment_plan"]`。
- 上游：judge_decision。
- 下游：交易员决策输入。
<details>
<summary>原始输出（保持原样）</summary>
...
## 9. 交易员决策（trader_investment_plan）
- 功能：交易员基于计划给出交易提案，写入 `state["trader_investment_plan"]`。
- 上游：investment_plan。
- 下游：触发第二轮风险辩论。
<details>
<summary>原始输出（保持原样）</summary>
...
## 10. 激进分析师发言（risky_history）
- 功能：针对首轮计划提出高风险偏好观点，写入 `state["risky_history"]`。
- 上游：此前的全部信息与提案。
- 下游：风险裁判使用。
<details>
<summary>原始输出（保持原样）</summary>
...
## 11. 保守分析师发言（safe_history）
- 功能：以资产安全为核心的反驳，写入 `state["safe_history"]`。
- 上游：同上。
- 下游：风险裁判使用。
<details>
<summary>原始输出（保持原样）</summary>
...
## 12. 中立分析师发言（neutral_history）
- 功能：提供折中建议，写入 `state["neutral_history"]`。
- 上游：同上。
- 下游：风险裁判使用。
<details>
<summary>原始输出（保持原样）</summary>
...
- 功能：基于三方风险偏好输出综合意见，写入 `state["risk_judge_decision"]`。
- 上游：risky/safe/neutral history。
- 下游：最终交易决策的直接输入。
<details>
<summary>原始输出（保持原样）</summary>
...
- 功能：汇总全部节点，输出最终指令并写入 `state["final_trade_decision"]`，流程收敛。
<details>
<summary>原始输出（保持原样）</summary>
