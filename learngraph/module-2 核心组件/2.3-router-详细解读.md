...
|---------|---------------------|------------------|---------|
| **Router** | LLM 作为决策中心的设计模式，动态选择执行路径 | 图结构中使用条件边实现智能路由，根据 LLM 输出决定下一步 | ⭐⭐⭐⭐⭐ |
| **tools_condition** | LangGraph 预定义的条件函数，检测是否需要调用工具 | 检查 AIMessage 是否包含 tool_calls 属性，返回 "tools" 或 END | ⭐⭐⭐⭐⭐ |
| **ToolNode** | LangGraph 预构建节点，自动执行工具调用 | 解析 tool_calls，调用对应函数，返回 ToolMessage | ⭐⭐⭐⭐⭐ |
| **bind_tools()** | 将工具绑定到 LLM，让模型知道可用工具 | ChatModel 方法，不执行工具只返回调用意图 | ⭐⭐⭐⭐⭐ |
| **tool_calls** | AI 消息中的工具调用请求列表 | AIMessage 属性，包含 name、args、id 等信息 | ⭐⭐⭐⭐⭐ |
| **Conditional Edge** | 根据状态动态路由到不同节点的边 | add_conditional_edges 方法添加，需提供条件函数 | ⭐⭐⭐⭐⭐ |
| **@tool 装饰器** | LangChain 工具装饰器，标记函数为工具 | 简化工具定义，自动处理函数签名和文档字符串 | ⭐⭐⭐⭐ |
| **parallel_tool_calls** | 控制是否允许并行调用多个工具的参数 | bind_tools 参数，False 时强制顺序执行工具调用 | ⭐⭐⭐⭐ |
| **Docstring** | 函数文档字符串，LLM 理解工具用途的唯一途径 | Python 字符串，描述函数功能和参数，影响 LLM 工具选择准确性 | ⭐⭐⭐⭐⭐ |
| **END** | LangGraph 的特殊终止节点 | 条件函数可返回 END 来结束图执行 | ⭐⭐⭐⭐⭐ |
...
![Router Architecture](https://cdn.prod.website-files.com/65b8cd72835ceeacd4449a53/66dbac6543c3d4df239a4ed1_router1.png)
...
![Router Flow Diagram](images/router-output-6-0.png)
