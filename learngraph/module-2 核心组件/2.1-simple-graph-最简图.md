...
|---------|---------------------|------------------|---------|
| **StateGraph** | LangGraph 的图构建器，用于创建和管理有状态的工作流 | 泛型类，接受状态类型作为参数，提供 add_node/add_edge 等方法 | ⭐⭐⭐⭐⭐ |
| **TypedDict** | 定义图状态的数据结构，提供类型安全的字典定义 | Python 3.8+ 的类型提示工具，用于声明字典的键值类型 | ⭐⭐⭐⭐⭐ |
| **Node（节点）** | 执行具体操作的函数单元，接收状态并返回状态更新 | Python 函数，必须接收 state 参数，返回字典用于更新状态 | ⭐⭐⭐⭐⭐ |
| **Edge（边）** | 连接节点的路径，定义工作流的执行顺序 | 使用 add_edge 方法添加，指定源节点和目标节点 | ⭐⭐⭐⭐⭐ |
| **Conditional Edge** | 根据状态动态选择下一个节点的条件边 | 使用 add_conditional_edges 方法，需提供条件函数返回目标节点名 | ⭐⭐⭐⭐⭐ |
| **START** | LangGraph 图的特殊入口节点，接收用户输入 | 内置常量，标记工作流开始位置，不是字符串 | ⭐⭐⭐⭐⭐ |
| **END** | LangGraph 图的特殊出口节点，标记执行结束 | 内置常量，标记工作流结束位置，返回最终状态 | ⭐⭐⭐⭐⭐ |
| **Literal** | 限定函数返回值的可能取值范围 | Python typing 模块的类型，用于条件函数的返回类型注解 | ⭐⭐⭐⭐ |
| **invoke()** | 同步执行图并返回最终结果的方法 | 阻塞调用，等待图执行完成后返回完整状态字典 | ⭐⭐⭐⭐⭐ |
| **compile()** | 将图构建器转换为可执行图的方法 | 验证图结构合法性，返回 CompiledGraph 对象 | ⭐⭐⭐⭐⭐ |
| **State** | 在节点间传递和共享的数据容器 | TypedDict 定义的字典结构，存储工作流的所有数据 | ⭐⭐⭐⭐⭐ |
| **Reducer** | 控制状态更新方式的函数（高级概念，后续课程详解） | 接收旧值和新值，返回合并后的值，用于非覆盖式更新 | ⭐⭐⭐ |
...
![Simple Graph Architecture](https://cdn.prod.website-files.com/65b8cd72835ceeacd4449a53/66dba5f465f6e9a2482ad935_simple-graph1.png)
...
状态是 LangGraph 的**核心数据结构**（本质是一个字典），贯穿整个图的执行过程。
...
![Simple Graph Flow Diagram](images/simple-graph-output-10-0.jpg)
