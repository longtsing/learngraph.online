...
|---------|---------------------|------------------|---------|
| **LangGraph Cloud** | LangGraph API 的托管服务，支持从 GitHub 自动部署 | 提供唯一 URL、监控追踪、环境变量管理的云平台 | ⭐⭐⭐⭐⭐ |
| **LangGraph Studio** | 可视化 IDE，用于测试和调试 LangGraph 应用 | Web 界面，支持本地/云端模式，实时预览图结构和状态 | ⭐⭐⭐⭐⭐ |
| **LangGraph SDK** | Python 客户端库，用于程序化调用 LangGraph API | 提供 assistants、threads、runs 等 API，支持异步操作 | ⭐⭐⭐⭐⭐ |
| **langgraph.json** | LangGraph 部署配置文件 | 定义图路径、依赖、环境变量等部署参数 | ⭐⭐⭐⭐⭐ |
| **Thread** | 对话会话的状态容器，支持多用户并发 | 通过 client.threads.create() 创建，用 thread_id 标识 | ⭐⭐⭐⭐⭐ |
| **Assistant** | 图的运行时实例，对应一个已部署的图 | 由 graph_id 标识，可有多个版本和配置 | ⭐⭐⭐⭐⭐ |
| **stream_mode** | 控制流式输出格式的参数 | values（完整状态）/updates（增量）/messages（仅消息） | ⭐⭐⭐⭐ |
| **async/await** | Python 异步编程语法，用于非阻塞 I/O | SDK 所有方法都是异步的，需使用 await 调用 | ⭐⭐⭐⭐ |
| **langgraph dev** | 启动本地开发服务器的命令 | 支持热重载，在 http://127.0.0.1:2024 提供 API | ⭐⭐⭐⭐⭐ |
| **getpass** | Python 安全输入模块，隐藏敏感信息 | getpass.getpass() 输入时不显示字符，防止密码泄露 | ⭐⭐⭐⭐ |
...
以下是完整的 SDK 使用代码，需要先启动本地服务器（`langgraph dev`）：
