...
|---------|---------------------|------------------|---------|
| **Checkpointer** | 在每个节点执行后自动保存状态的持久化机制 | 接口类，提供 save/load checkpoint 方法，支持状态恢复 | ⭐⭐⭐⭐⭐ |
| **MemorySaver** | 基于内存的检查点器，用于开发和测试 | Checkpointer 实现，数据存储在进程内存中，重启后丢失 | ⭐⭐⭐⭐⭐ |
| **Thread** | 独立的对话会话容器，隔离不同用户/会话的状态 | 通过 thread_id 标识，每个 thread 维护独立的 checkpoint 历史 | ⭐⭐⭐⭐⭐ |
| **thread_id** | 线程的唯一标识符，用于区分不同会话 | config["configurable"]["thread_id"] 配置项，字符串类型 | ⭐⭐⭐⭐⭐ |
| **Config** | 运行时配置字典，传递 thread_id 等参数 | invoke/stream 方法的 config 参数，包含 configurable 字段 | ⭐⭐⭐⭐⭐ |
| **Checkpoint** | 某个时间点的完整状态快照 | 包含 messages 等状态字段的序列化数据，可用于恢复 | ⭐⭐⭐⭐ |
| **PostgresSaver** | 基于 PostgreSQL 数据库的生产级检查点器 | Checkpointer 实现，支持持久化存储和分布式部署 | ⭐⭐⭐⭐ |
| **add_messages** | MessagesState 使用的智能消息合并 reducer | 自动追加新消息，相同 ID 的消息会被替换而非追加 | ⭐⭐⭐⭐⭐ |
| **MessagesState** | 预定义的消息状态类，包含 messages 字段 | TypedDict 子类，自动使用 add_messages reducer | ⭐⭐⭐⭐⭐ |
| **compile(checkpointer)** | 编译图时注入检查点器的方法 | builder.compile(checkpointer=memory) 启用状态持久化 | ⭐⭐⭐⭐⭐ |
...
![Flow Diagram](images/agent-memory-output-9-0.jpg)
