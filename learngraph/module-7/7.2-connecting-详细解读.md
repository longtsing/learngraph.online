...
|---------|---------------------|------------------|---------|
| **LangGraph SDK** | 官方 Python/JavaScript 客户端库，提供 runs、threads、store、assistants 等 API，用于连接和操作部署的图。 | 通过 `pip install langgraph_sdk` 安装，使用 `get_client(url)` 创建客户端对象。 | ⭐⭐⭐⭐⭐ |
| **Remote Graph** | 允许像使用本地图一样使用远程部署图的接口，提供 invoke/stream 等方法。 | 从 `langgraph.pregel.remote` 导入，适合快速原型开发和最小化代码改动。 | ⭐⭐⭐⭐ |
| **Run** | 图的一次完整执行，有唯一 ID，结果存储在数据库，状态包括 pending/running/success/error。 | 通过 `client.runs.create()` 创建，支持后台运行、阻塞运行和流式运行三种模式。 | ⭐⭐⭐⭐⭐ |
| **Thread** | 多轮交互的会话容器，保存对话历史、状态和检查点，支持状态持久化。 | 通过 `client.threads.create()` 创建，每个 thread 有唯一 thread_id，存储在 PostgreSQL。 | ⭐⭐⭐⭐⭐ |
| **Background Run** | 后台运行模式，立即返回不等待完成，适合长时间任务，可轮询状态。 | `client.runs.create()` 默认模式，返回 run_id 后立即返回，图在后台异步执行。 | ⭐⭐⭐⭐ |
| **Blocking Run** | 阻塞运行模式，等待图执行完成后返回，确保顺序执行。 | 使用 `client.runs.join(thread_id, run_id)` 等待完成，类似于同步调用。 | ⭐⭐⭐⭐ |
| **Streaming Run** | 流式运行模式，实时接收执行过程中的更新和 token，改善用户体验。 | 使用 `client.runs.stream()` 并指定 stream_mode（如 messages-tuple），返回异步迭代器。 | ⭐⭐⭐⭐⭐ |
| **Checkpoint** | 图执行过程中的状态快照，每个节点执行后创建，支持时间旅行和 Human-in-the-loop。 | 包含 checkpoint_id、values、next 等字段，存储在 PostgreSQL 的 checkpoints 表。 | ⭐⭐⭐⭐⭐ |
| **Time Travel** | 回到线程历史中的任意检查点，编辑状态后从该点继续执行，实现状态分支。 | 通过 `threads.get_history()` 获取历史，`threads.update_state()` 编辑状态，指定 checkpoint_id 继续执行。 | ⭐⭐⭐⭐ |
| **Store API** | 跨线程的长期记忆存储，按 namespace 组织，支持搜索、添加、删除操作。 | 提供 `store.search_items()`、`store.put_item()`、`store.delete_item()` 等方法，数据存储在 PostgreSQL。 | ⭐⭐⭐⭐⭐ |
| **Namespace** | Store 中数据的组织单位，使用元组表示，如 ("todo", "general", "user_id")。 | Python 元组类型，支持多层级组织，用于隔离不同用户、类别的数据。 | ⭐⭐⭐⭐ |
| **stream_mode** | 流式传输的模式选项，控制返回的数据格式，包括 values/updates/messages/messages-tuple 等。 | 字符串参数，messages-tuple 模式适合逐 token 流式传输，values 模式返回完整状态。 | ⭐⭐⭐⭐ |
