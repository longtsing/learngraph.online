...
|---------|---------------------|------------------|---------|
| **Profile Schema** | 单一结构化对象的数据模式，定义固定字段（如姓名、位置、兴趣），使用固定键存储 | 使用 Pydantic BaseModel 定义，如 `UserProfile(user_name: str, interests: List[str])` | ⭐⭐⭐⭐⭐ |
| **TypedDict** | Python 的类型化字典，提供类型提示但不在运行时验证数据 | `typing.TypedDict` 类，定义字典的键和值类型，如 `class UserProfile(TypedDict): user_name: str` | ⭐⭐⭐ |
| **Pydantic BaseModel** | 带运行时数据验证的 Python 数据模型，支持类型检查、默认值和序列化 | 继承 `pydantic.BaseModel` 的类，自动验证字段类型和值，如 `class UserProfile(BaseModel)` | ⭐⭐⭐⭐⭐ |
| **Field** | Pydantic 中定义字段的函数，可添加描述、默认值和验证规则 | `pydantic.Field(description="...", default=..., ge=0)`，为 LLM 提供上下文 | ⭐⭐⭐⭐ |
| **with_structured_output()** | LangChain 方法，强制 LLM 输出符合预定义 Schema 的结构化数据 | `model.with_structured_output(Schema)` 返回 Runnable，自动解析为 Python 对象 | ⭐⭐⭐⭐⭐ |
| **Trustcall** | 开源库，专门用于创建和更新 JSON Schema，支持增量更新和自我修正 | `from trustcall import create_extractor`，处理复杂嵌套 Schema 和 JSON Patch | ⭐⭐⭐⭐⭐ |
| **JSON Patch** | RFC 6902 标准，描述 JSON 文档变更的操作列表（add、remove、replace 等） | 操作数组，如 `[{"op": "add", "path": "/interests/-", "value": "bakeries"}]` | ⭐⭐⭐⭐ |
| **create_extractor()** | Trustcall 函数，创建结构化数据提取器，支持工具调用和增量更新 | `create_extractor(model, tools=[Schema], tool_choice="Schema")`，返回可调用的提取器 | ⭐⭐⭐⭐⭐ |
| **model_dump()** | Pydantic 模型方法，将对象转换为 Python 字典，便于存储和序列化 | `pydantic_obj.model_dump()` 返回 dict，Pydantic v2 替代旧版 `dict()` 方法 | ⭐⭐⭐⭐ |
| **增量更新（Incremental Update）** | 只更新变化的字段而不是重新生成整个对象，使用 JSON Patch 实现 | Trustcall 自动识别变化并生成最小化的更新操作，节省 Token 和保留信息 | ⭐⭐⭐⭐⭐ |
| **existing 参数** | Trustcall 的关键参数，传入现有数据以触发更新模式而非创建模式 | 格式：`{"existing": {"SchemaName": existing_data}}`，告诉 Trustcall 执行增量更新 | ⭐⭐⭐⭐ |
| **自我修正（Self-Correction）** | Trustcall 内置机制，当数据验证失败时自动重试并修正错误 | 自动捕获 ValidationError，生成修正提示并重新调用 LLM，提高成功率 | ⭐⭐⭐⭐ |
...
下面是一个完整的 Profile Schema 示例，演示如何使用 Pydantic 定义结构化数据模式并与 Store 配合使用：

```python
"""
LangGraph Profile Schema 完整示例
演示：Pydantic 结构化数据 + InMemoryStore 长期记忆
"""
from typing import Optional
from pydantic import BaseModel, Field
from langgraph.graph import StateGraph, MessagesState, START, END
...
print("=" * 50)
print("Pydantic Profile Schema 演示")
print("=" * 50)
...
# 转换为字典（用于存储）
profile_dict = profile1.model_dump()
print(f"\n转换为字典: {profile_dict}")

# 从字典创建（从存储加载）
profile2 = UserProfile(**profile_dict)
print(f"\n从字典恢复: {profile2}")

# 增量更新（创建新对象）
updated_profile = profile1.model_copy(update={"occupation": "软件工程师"})
print(f"\n增量更新后: {updated_profile}")

# ============ 4. 定义节点函数 ============

def parse_user_info(message: str, existing_profile: dict) -> dict:
    """简单解析用户信息（实际应用中可用 LLM）"""
    profile = existing_profile.copy()

    if "我叫" in message or "名字是" in message:
        for keyword in ["我叫", "名字是"]:
            if keyword in message:
                name = message.split(keyword)[-1].split("，")[0].split("。")[0].strip()
                profile["user_name"] = name
                break

    if "住在" in message or "在" in message and "工作" not in message:
        for keyword in ["住在", "在"]:
            if keyword in message:
                location = message.split(keyword)[-1].split("，")[0].split("。")[0].strip()
                if len(location) < 10:  # 简单过滤
                    profile["user_location"] = location
                break

    if "喜欢" in message:
        interest = message.split("喜欢")[-1].split("，")[0].split("。")[0].strip()
        if interest and interest not in profile.get("interests", []):
            profile.setdefault("interests", []).append(interest)

    if "职业是" in message or "工作是" in message or "是一名" in message:
        for keyword in ["职业是", "工作是", "是一名"]:
            if keyword in message:
                occupation = message.split(keyword)[-1].split("，")[0].split("。")[0].strip()
...
def chatbot(state: MessagesState, config: RunnableConfig, *, store: BaseStore):
    ...
    # 解析用户信息并更新 Profile
    updated_profile = parse_user_info(last_msg, profile_dict)

    # 检查是否有更新
    if updated_profile != profile_dict:
        # 验证并保存
        try:
            validated = UserProfile(**updated_profile)
            store.put(namespace, "profile", validated.model_dump())
            profile_dict = validated.model_dump()
        except Exception as e:
            print(f"验证错误: {e}")

    # 生成响应
    user_name = profile_dict.get("user_name") or "朋友"

    if "你好" in last_msg or "hi" in last_msg.lower():
        response = f"你好 {user_name}！"
        if profile_dict.get("user_location"):
            response += f"我知道你在{profile_dict['user_location']}。"
        response += "有什么我可以帮助你的吗？"
    elif "我的资料" in last_msg or "我是谁" in last_msg:
        profile = UserProfile(**profile_dict)
        parts = []
        if profile.user_name:
            parts.append(f"姓名: {profile.user_name}")
        if profile.user_location:
            parts.append(f"位置: {profile.user_location}")
        if profile.interests:
            parts.append(f"兴趣: {', '.join(profile.interests)}")
        if profile.occupation:
            parts.append(f"职业: {profile.occupation}")
        response = "你的资料如下：\n" + "\n".join(parts) if parts else "我还不太了解你，请告诉我更多信息。"
    elif any(kw in last_msg for kw in ["我叫", "住在", "喜欢", "职业是", "工作是"]):
        response = f"好的 {user_name}，我已经更新了你的资料！"
    else:
        response = f"{user_name}，我理解了。还有什么想告诉我的吗？"
...
config = {"configurable": {"thread_id": "test1", "user_id": "user_001"}}
...
print(f"Human: 你好，我叫张三")
print(f"AI: {result['messages'][-1].content}")

# 对话 2: 介绍位置
result = graph.invoke(
    {"messages": [HumanMessage(content="我住在北京")]},
    config
)
print(f"\nHuman: 我住在北京")
print(f"AI: {result['messages'][-1].content}")
...
# 对话 3: 介绍兴趣
result = graph.invoke(
    {"messages": [HumanMessage(content="我喜欢人工智能")]},
    config
)
print(f"\nHuman: 我喜欢人工智能")
print(f"AI: {result['messages'][-1].content}")
...
# 对话 4: 介绍职业
result = graph.invoke(
    {"messages": [HumanMessage(content="我的职业是数据科学家")]},
    config
)
print(f"\nHuman: 我的职业是数据科学家")
print(f"AI: {result['messages'][-1].content}")
...
result = graph.invoke(
    {"messages": [HumanMessage(content="请显示我的资料")]},
    config
)
print(f"Human: 请显示我的资料")
print(f"AI: {result['messages'][-1].content}")

print("\n" + "=" * 50)
print("测试 3: 验证 Store 中的结构化数据")
print("=" * 50)

profile_item = store.get(("profiles", "user_001"), "profile")
if profile_item:
    print(f"\n存储的原始数据: {profile_item.value}")

    # 转换为 Pydantic 模型
    profile = UserProfile(**profile_item.value)
    print(f"\nPydantic 模型: {profile}")
    print(f"  - 姓名: {profile.user_name}")
    print(f"  - 位置: {profile.user_location}")
...
```
==================================================
Pydantic Profile Schema 演示
==================================================

创建 Profile: user_name='小明' user_location='上海' interests=['编程', '阅读'] occupation=None

转换为字典: {'user_name': '小明', 'user_location': '上海', ...}
...
==================================================
测试 3: 验证 Store 中的结构化数据
==================================================

存储的原始数据: {'user_name': '张三', 'user_location': '北京', 'interests': ['人工智能'], 'occupation': '数据科学家'}

Pydantic 模型: user_name='张三' user_location='北京' interests=['人工智能'] occupation='数据科学家'
  - 姓名: 张三
  - 位置: 北京
  - 兴趣: ['人工智能']
  - 职业: 数据科学家
...
| 概念 | 说明 | 代码示例 |
|------|------|---------|
| **Pydantic BaseModel** | 定义结构化数据模型 | `class UserProfile(BaseModel)` |
| **Field** | 定义字段元数据 | `Field(description="...")` |
| **model_dump()** | 转换为字典 | `profile.model_dump()` |
| **model_copy()** | 创建更新副本 | `profile.model_copy(update={...})` |
| **数据验证** | 自动类型检查 | Pydantic 自动验证 |
| **增量更新** | 只更新变化的字段 | 避免重新生成整个对象 |
