...
|---------|---------------------|------------------|---------|
| **Memory Agent（记忆 Agent）** | 具备记忆能力的智能体，可跨会话保存和访问用户信息、偏好和历史数据 | 使用 LangGraph 的 Store 和 Checkpointer 实现，支持语义、情节和程序性记忆 | ⭐⭐⭐⭐⭐ |
| **task_mAIstro** | 本节构建的待办事项管理 Agent，整合 Profile、ToDo Collection 和 Instructions | 使用 ReAct 架构、条件路由和多类型 Trustcall 工具的完整 Agent 实现 | ⭐⭐⭐⭐ |
| **ReAct 架构** | Reasoning + Acting 模式，Agent 循环推理和执行工具直到完成任务 | 节点流程：`agent → tools → agent → ...`，通过 `should_continue()` 判断是否继续 | ⭐⭐⭐⭐⭐ |
| **UpdateMemory 工具** | Trustcall 提供的特殊工具类，用于在 Agent 中更新结构化记忆 | `from trustcall import UpdateMemory`，配置 Schema 和 enable_inserts，在工具列表中使用 | ⭐⭐⭐⭐⭐ |
| **语义记忆（Semantic）** | 存储事实和知识的记忆类型，如 Profile（用户资料）和 ToDo Collection（任务） | Pydantic 模型定义，如 `UserProfile(name, location)` 和 `ToDo(task, deadline)` | ⭐⭐⭐⭐⭐ |
| **程序性记忆（Procedural）** | 存储操作方法的记忆类型，如 Instructions（用户偏好的任务创建方式） | Pydantic 模型：`Instructions(task_creation_instructions: str)`，指导 Agent 行为 | ⭐⭐⭐⭐ |
| **情节记忆（Episodic）** | 存储特定事件和经历的记忆类型，如对话历史和时间序列事件 | 通过 Checkpointer 保存对话历史，或在 Collection 中按时间戳存储事件 | ⭐⭐⭐⭐ |
| **条件边（Conditional Edges）** | 根据状态动态决定下一个节点的边，用于实现智能路由和决策 | `add_conditional_edges(source, route_fn, edge_mapping)`，如判断工具调用结果 | ⭐⭐⭐⭐⭐ |
| **should_continue()** | 路由函数，判断 Agent 是否应继续执行工具还是结束 | 检查 `state["messages"][-1].tool_calls`，有则返回 "tools"，否则返回 END | ⭐⭐⭐⭐⭐ |
| **bind_tools()** | LangChain 方法，将工具列表绑定到 LLM，使其可通过工具调用与外部交互 | `model.bind_tools([tool1, tool2, ...])`，LLM 输出包含 tool_calls 字段 | ⭐⭐⭐⭐⭐ |
| **ToolNode** | LangGraph 预构建节点，自动执行工具调用并返回结果 | `from langgraph.prebuilt import ToolNode`，处理 AIMessage 中的 tool_calls | ⭐⭐⭐⭐ |
| **多类型记忆协同** | Profile、Collection 和 Instructions 三种记忆类型同时使用，互相补充 | 不同 namespace 分别存储：`(user_id, "profile")`、`(user_id, "todos")`、`(user_id, "instructions")` | ⭐⭐⭐⭐⭐ |
...
![Memory Agent Architecture](https://cdn.prod.website-files.com/65b8cd72835ceeacd4449a53/6732cfb05d9709862eba4e6c_Screenshot%202024-11-11%20at%207.46.40%E2%80%AFPM.png)
...
下面是一个简化但完整的 Memory Agent 示例，演示短期记忆（Checkpointer）和长期记忆（Store）的结合使用：

```python
"""
LangGraph Memory Agent 完整示例
演示：短期记忆（Checkpointer）+ 长期记忆（Store）+ ReAct 架构
"""
import os
...
class ToDo(BaseModel):
    """待办事项"""
    task: str = Field(description="任务描述")
    status: Literal["pending", "done"] = Field(default="pending")

class UpdateMemory(TypedDict):
    """更新记忆工具参数"""
    update_type: Literal["profile", "todo"]

# ============ 2. 节点函数定义 ============

def task_agent(state: MessagesState, config: RunnableConfig, *, store: InMemoryStore):
    """主 Agent 节点：处理用户输入，决定是否更新记忆"""
    user_id = config["configurable"].get("user_id", "default")

    # 从 Store 获取长期记忆
    profile_items = list(store.search(("profile", user_id)))
    profile = profile_items[0].value if profile_items else {"name": None, "location": None, "interests": []}

    todo_items = list(store.search(("todos", user_id)))
    todos = [item.value for item in todo_items]

    # 构建系统消息
    system_msg = f"""你是一个智能助手，帮助用户管理待办事项。

当前用户资料：
- 姓名：{profile.get('name', '未知')}
- 位置：{profile.get('location', '未知')}
- 兴趣：{', '.join(profile.get('interests', [])) or '未知'}

当前待办事项：
{chr(10).join([f"- {t.get('task')} [{t.get('status')}]" for t in todos]) or '无'}

指令：
1. 如果用户分享个人信息，调用 UpdateMemory(update_type="profile")
2. 如果用户提到任务/待办事项，调用 UpdateMemory(update_type="todo")
3. 否则正常对话
"""

    messages = [SystemMessage(content=system_msg)] + state["messages"]

    # 简化实现：根据消息内容判断是否需要更新记忆
    last_msg = state["messages"][-1].content.lower()

    if any(word in last_msg for word in ["我叫", "住在", "喜欢", "我是", "我在"]):
        # 需要更新 Profile
        response = AIMessage(
            content="",
            tool_calls=[{
                "id": "call_profile",
                "name": "UpdateMemory",
                "args": {"update_type": "profile"}
            }]
        )
    elif any(word in last_msg for word in ["任务", "待办", "要做", "需要", "完成了"]):
        # 需要更新 ToDo
        response = AIMessage(
            content="",
            tool_calls=[{
        ...
        # 普通对话
        profile_name = profile.get('name', '朋友')
        response = AIMessage(content=f"你好{profile_name}！有什么我可以帮助你的吗？")
...
def update_profile(state: MessagesState, config: RunnableConfig, *, store: InMemoryStore):
    """更新用户 Profile"""
    user_id = config["configurable"].get("user_id", "default")
    last_msg = state["messages"][-2].content  # 获取用户消息

    # 简单解析用户信息
    profile = {"name": None, "location": None, "interests": []}

    if "我叫" in last_msg:
        name_part = last_msg.split("我叫")[-1].split("，")[0].split("。")[0].strip()
        profile["name"] = name_part
    if "住在" in last_msg:
        location_part = last_msg.split("住在")[-1].split("，")[0].split("。")[0].strip()
        profile["location"] = location_part
    if "喜欢" in last_msg:
        interest_part = last_msg.split("喜欢")[-1].split("，")[0].split("。")[0].strip()
        profile["interests"] = [interest_part]

    # 保存到 Store
    store.put(("profile", user_id), "info", profile)

    from langchain_core.messages import ToolMessage
    tool_msg = ToolMessage(content=f"已更新用户资料：{profile}", tool_call_id="call_profile")
    confirm_msg = AIMessage(content=f"好的，我已经记住你的信息了！")

    return {"messages": [tool_msg, confirm_msg]}

def update_todo(state: MessagesState, config: RunnableConfig, *, store: InMemoryStore):
    """更新 ToDo 列表"""
    user_id = config["configurable"].get("user_id", "default")
    ...
    if "完成了" in last_msg:
        ...
        for item in todo_items:
            if item.value.get("task") in last_msg:
                item.value["status"] = "done"
                store.put(("todos", user_id), item.key, item.value)
        result = "任务已标记为完成！"
    else:
        # 添加新任务
        task_desc = last_msg.replace("任务", "").replace("待办", "").replace("要做", "").replace("需要", "").strip()
        if task_desc.startswith("：") or task_desc.startswith("："):
            task_desc = task_desc[1:].strip()

        todo = {"task": task_desc, "status": "pending"}
        store.put(("todos", user_id), str(uuid.uuid4()), todo)
        result = f"已添加任务：{task_desc}"

    from langchain_core.messages import ToolMessage
    tool_msg = ToolMessage(content=result, tool_call_id="call_todo")
    confirm_msg = AIMessage(content=result)
...
builder.add_conditional_edges("task_agent", should_continue, {
    "update_profile": "update_profile",
    "update_todo": "update_todo",
    END: END
})
builder.add_edge("update_profile", END)
builder.add_edge("update_todo", END)
...
print("\n" + "=" * 50)
print("测试 1: 保存用户资料（长期记忆）")
print("=" * 50)

config1 = {"configurable": {"thread_id": "session1", "user_id": "alice"}}

# 用户介绍自己
result = graph.invoke(
    {"messages": [HumanMessage(content="你好！我叫小明，住在上海，喜欢编程")]},
    config1
)
...
print("\n" + "=" * 50)
print("测试 2: 添加待办事项（长期记忆）")
print("=" * 50)

result = graph.invoke(
    {"messages": [HumanMessage(content="我需要完成 LangGraph 教程")]},
    config1
)
for msg in result["messages"]:
    if hasattr(msg, "content") and msg.content:
...
print("\n" + "=" * 50)
print("测试 4: 查看存储的长期记忆")
print("=" * 50)

print("\n用户资料:")
for item in store.search(("profile", "alice")):
    print(f"  {item.value}")

print("\n待办事项:")
for item in store.search(("todos", "alice")):
    print(f"  {item.value}")
```
...
graph TD;
	...
	__start__ --> task_agent;
	update_profile --> __end__;
	update_todo --> __end__;
	task_agent -.-> update_profile;
	task_agent -.-> update_todo;
	task_agent -.-> __end__;

==================================================
测试 1: 保存用户资料（长期记忆）
==================================================
HumanMessage: 你好！我叫小明，住在上海，喜欢编程
ToolMessage: 已更新用户资料：{'name': '小明', 'location': '上海', 'interests': ['编程']}
AIMessage: 好的，我已经记住你的信息了！

==================================================
测试 2: 添加待办事项（长期记忆）
==================================================
HumanMessage: 我需要完成 LangGraph 教程
ToolMessage: 已添加任务：完成 LangGraph 教程
AIMessage: 已添加任务：完成 LangGraph 教程

==================================================
测试 3: 跨会话访问记忆
==================================================
HumanMessage: 你好
AIMessage: 你好小明！有什么我可以帮助你的吗？
...
用户资料:
  {'name': '小明', 'location': '上海', 'interests': ['编程']}

待办事项:
  {'task': '完成 LangGraph 教程', 'status': 'pending'}
```
...
| 概念 | 说明 | 代码示例 |
|------|------|---------|
| **短期记忆** | 使用 `MemorySaver` 保存对话历史 | `checkpointer = MemorySaver()` |
| **长期记忆** | 使用 `InMemoryStore` 保存跨会话数据 | `store = InMemoryStore()` |
| **命名空间** | 用元组组织数据 | `("profile", user_id)`, `("todos", user_id)` |
| **Store API** | `put` 保存, `search` 查询 | `store.put(namespace, key, value)` |
| **条件路由** | 根据状态决定下一节点 | `add_conditional_edges(node, router_fn, mapping)` |
| **ReAct 架构** | Agent 推理 → 工具调用 → 响应 | `task_agent → update_* → END` |
