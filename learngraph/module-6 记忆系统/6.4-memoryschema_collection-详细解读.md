...
|---------|---------------------|------------------|---------|
| **Collection Schema** | 多个独立记忆项的集合模式，每个项目使用 UUID 键独立存储，支持无限扩展 | 使用 Pydantic BaseModel 定义单个项目结构，如 `Memory(content: str)` | ⭐⭐⭐⭐⭐ |
| **UUID（通用唯一识别码）** | 全局唯一的 128 位标识符，用于标识 Collection 中的每个独立记忆项 | `uuid.uuid4()` 生成，转换为字符串作为 Store 的键，如 `str(uuid.uuid4())` | ⭐⭐⭐⭐⭐ |
| **enable_inserts** | Trustcall 参数，设为 True 时允许插入新的独立项目（Collection 模式必需） | `create_extractor(model, tools=[Memory], enable_inserts=True)`，支持并行创建多个对象 | ⭐⭐⭐⭐⭐ |
| **json_doc_id** | Trustcall 返回元数据中的字段，标识更新操作的目标对象 ID | 在 `response_metadata` 中，如 `{"id": "...", "json_doc_id": "existing_key"}`，有则更新，无则插入 | ⭐⭐⭐⭐⭐ |
| **并行工具调用（Parallel Tool Calling）** | Trustcall 同时调用多个工具实例，处理更新和插入操作 | 一次调用返回多个响应，如 `result["responses"]` 包含多个 Memory 对象 | ⭐⭐⭐⭐ |
| **existing 格式（Collection）** | Collection 模式下 Trustcall 的 existing 参数格式，使用列表的三元组 | 格式：`[(key, "ToolName", data), ...]`，如 `[("uuid1", "Memory", {...}), ("uuid2", "Memory", {...})]` | ⭐⭐⭐⭐⭐ |
| **merge_message_runs()** | LangChain 函数，合并连续的相同角色消息，减少消息数量 | `from langchain_core.messages import merge_message_runs`，合并如 `[HumanMessage, HumanMessage]` 为单个消息 | ⭐⭐⭐ |
| **Profile vs Collection** | Profile 是单一对象（固定键），Collection 是多个独立项（UUID 键） | Profile 适合固定结构（用户资料），Collection 适合开放式扩展（笔记、任务） | ⭐⭐⭐⭐⭐ |
| **search() 遍历** | Collection 模式下使用 search() 获取所有记忆项，而非 get() 单个对象 | `store.search(namespace)` 返回列表，需遍历：`for item in items: item.value["content"]` | ⭐⭐⭐⭐ |
| **rmeta.get()** | Python 字典的安全获取方法，用于判断是更新还是插入操作 | `rmeta.get("json_doc_id", str(uuid.uuid4()))`，有 json_doc_id 用现有键，否则生成新 UUID | ⭐⭐⭐⭐⭐ |
| **zip() 函数** | Python 内置函数，并行迭代多个序列，用于同时处理响应和元数据 | `for r, rmeta in zip(responses, metadata)`，配对处理每个响应及其元数据 | ⭐⭐⭐⭐ |
| **格式化生成器** | Python 生成器表达式，用于高效格式化记忆列表 | `"\n".join(f"- {mem.value['content']}" for mem in memories)`，一行代码格式化所有记忆 | ⭐⭐⭐ |
...
下面是一个完整的 Collection Schema 示例，演示如何使用 UUID 键存储多个独立的记忆项：

```python
"""
LangGraph Collection Schema 完整示例
演示：UUID 键 + 多个独立记忆项 + 动态扩展
"""
import uuid
from pydantic import BaseModel, Field
from langgraph.graph import StateGraph, MessagesState, START, END
...
print("=" * 50)
print("Collection Schema 演示")
print("=" * 50)

# 添加多个独立记忆项
namespace = ("memories", "demo_user")
...
for content in memories_to_add:
    key = str(uuid.uuid4())  # 每个记忆使用唯一 UUID
    store.put(namespace, key, {"content": content})
    print(f"  [{key[:8]}...]: {content}")
...
def extract_memories(message: str) -> list[str]:
    """从消息中提取可保存的记忆（简化实现）"""
    memories = []
    keywords = ["喜欢", "是", "住在", "工作", "学习", "想要", "计划", "正在"]
...
def chatbot(state: MessagesState, config: RunnableConfig, *, store: BaseStore):
    ...
    memories_text = "\n".join(f"- {m.value['content']}" for m in existing_memories)

    # 提取新记忆
    new_memories = extract_memories(last_msg)
    for content in new_memories:
        # 检查是否已存在相似记忆
        is_duplicate = any(content in m.value['content'] or m.value['content'] in content
    ...
    # 生成响应
    if "记忆" in last_msg or "记住" in last_msg:
        # 显示所有记忆
        all_memories = list(store.search(namespace))
        if all_memories:
            memory_list = "\n".join(f"- {m.value['content']}" for m in all_memories)
            response = f"我记住了以下关于你的信息：\n{memory_list}"
        else:
            response = "我还没有记住任何关于你的信息。"
    elif "删除" in last_msg:
        # 清空记忆
        for item in store.search(namespace):
            store.delete(namespace, item.key)
        response = "好的，我已经清除了所有记忆。"
    elif new_memories:
        response = f"好的，我记住了：{new_memories[0]}"
    else:
        if memories_text:
            response = f"根据我的记忆，我知道：\n{memories_text}\n\n有什么我可以帮助你的？"
        else:
            response = "你好！请告诉我一些关于你的信息，我会记住的。"
...
config = {"configurable": {"thread_id": "test1", "user_id": "test_user"}}

# 添加记忆 1
result = graph.invoke(
    {"messages": [HumanMessage(content="我喜欢打篮球")]},
    config
)
print(f"Human: 我喜欢打篮球")
print(f"AI: {result['messages'][-1].content}")

# 添加记忆 2
result = graph.invoke(
    {"messages": [HumanMessage(content="我是一名数据分析师")]},
    config
)
print(f"\nHuman: 我是一名数据分析师")
print(f"AI: {result['messages'][-1].content}")
...
# 添加记忆 3
result = graph.invoke(
    {"messages": [HumanMessage(content="我正在学习 LangGraph")]},
    config
)
print(f"\nHuman: 我正在学习 LangGraph")
print(f"AI: {result['messages'][-1].content}")
...
result = graph.invoke(
    {"messages": [HumanMessage(content="显示我的记忆")]},
    config
)
print(f"Human: 显示我的记忆")
print(f"AI: {result['messages'][-1].content}")

print("\n" + "=" * 50)
print("测试 3: 验证 Store 中的数据")
print("=" * 50)

print("\n存储的所有记忆项：")
for item in store.search(("memories", "test_user")):
    print(f"  Key: {item.key[:8]}...")
    print(f"  Value: {item.value}")
    print()

print(f"总记忆数: {len(list(store.search(('memories', 'test_user'))))}")
```
...
| 概念 | 说明 | 代码示例 |
|------|------|---------|
| **UUID 键** | 每个记忆使用唯一标识符 | `str(uuid.uuid4())` |
| **Collection** | 多个独立记忆项的集合 | `store.search(namespace)` |
| **动态扩展** | 可无限添加新记忆 | `store.put(ns, uuid, data)` |
| **search()** | 获取所有记忆项 | 遍历 namespace 下所有数据 |
| **delete()** | 删除特定记忆 | `store.delete(ns, key)` |
