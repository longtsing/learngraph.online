...
|---------|---------------------|------------------|---------|
| **Checkpointer（检查点器）** | LangGraph 的短期记忆机制，保存单次会话的对话历史和 Graph State | 如 `MemorySaver()`，通过 thread_id 标识会话，存储完整的执行状态快照 | ⭐⭐⭐⭐⭐ |
| **Store（存储）** | LangGraph 的长期记忆机制，跨会话持久化保存结构化数据 | 实现 BaseStore 接口，如 `InMemoryStore()` 或 `PostgresStore()`，使用 namespace+key 组织数据 | ⭐⭐⭐⭐⭐ |
| **语义记忆 vs 程序性记忆** | 语义记忆存储事实知识（Profile、Collection），程序性记忆存储操作方法（Instructions） | 语义记忆如 `{"name": "Lance"}`，程序性记忆如 `{"instruction": "包含本地商家信息"}` | ⭐⭐⭐⭐ |
| **Profile 模式** | 单一结构化对象，固定字段，适合用户资料和系统配置，使用固定键 + JSON Patch 更新 | 定义：`class UserProfile(BaseModel)`，存储：`store.put(ns, "user_profile", data)` | ⭐⭐⭐⭐⭐ |
| **Collection 模式** | 多个独立项集合，灵活扩展，适合笔记任务事件，每项使用 UUID 键 | 定义：`class Memory(BaseModel)`，存储：`store.put(ns, str(uuid.uuid4()), data)` | ⭐⭐⭐⭐⭐ |
| **Trustcall 的双模式** | Profile 模式（enable_inserts=False）和 Collection 模式（enable_inserts=True） | Profile 更新单一对象，Collection 可并行创建多个对象，通过 json_doc_id 区分更新/插入 | ⭐⭐⭐⭐⭐ |
| **Namespace 设计模式** | 多层命名空间实现数据隔离和组织，如 `(tenant_id, user_id, resource_type)` | Python 元组，第一层通常是最常用的查询维度，支持 3-5 层嵌套 | ⭐⭐⭐⭐ |
| **热路径 vs 冷路径** | 热路径在用户等待时同步保存记忆，冷路径使用后台异步处理 | 热路径简单但增加延迟，冷路径需要消息队列但不影响响应速度 | ⭐⭐⭐⭐ |
| **跨会话记忆** | 使用 Store 实现的记忆，不同 thread_id 的会话可共享，通过 user_id 关联 | 配置：`{"user_id": "1", "thread_id": "session_1"}`，Store 使用 user_id 命名空间 | ⭐⭐⭐⭐⭐ |
| **增量更新 vs 完全重建** | Trustcall 使用 JSON Patch 只更新变化部分，with_structured_output 完全重建对象 | JSON Patch 节省 Token 并保留未变化信息，适合复杂 Schema 和频繁更新 | ⭐⭐⭐⭐⭐ |
| **条件路由（Conditional Edges）** | 根据状态动态决定下一个节点，用于智能触发记忆更新 | `add_conditional_edges("agent", route_fn, {"update": "update_profile", END: END})` | ⭐⭐⭐⭐ |
| **多租户架构（Multi-tenancy）** | 企业级设计，通过 namespace 实现租户隔离和权限控制 | 命名空间：`(tenant_id, user_id, resource_type)`，结合角色权限模型（RBAC） | ⭐⭐⭐⭐ |
...
graph TB
    ...
    B1 --> B1a[语义记忆<br/>Semantic]
    B1 --> B1b[情节记忆<br/>Episodic]
    B1 --> B1c[程序性记忆<br/>Procedural]
    
    B2 --> B2a[Checkpointer<br/>会话内]
    B2 --> B2b[Store<br/>跨会话]
    ...
    C1 --> C1a[Namespace<br/>命名空间]
    C1 --> C1b[Key<br/>键]
    C1 --> C1c[Value<br/>值字典]
    C1 --> C1d[put/get/search<br/>基本操作]
    
    C2 --> C2a[结构化提取]
    C2 --> C2b[JSON Patch<br/>增量更新]
    C2 --> C2c[enable_inserts<br/>插入控制]
    
    C3 --> C3a[BaseModel<br/>Schema定义]
    C3 --> C3b[Field<br/>字段描述]
    C3 --> C3c[数据验证]
    ...
    D1 --> D1a[单一对象<br/>固定结构]
    D1 --> D1b[增量更新<br/>JSON Patch]
    D1 --> D1c[适用场景<br/>用户资料/配置]
    
    D2 --> D2a[多个独立项<br/>灵活结构]
    D2 --> D2b[UUID键管理]
    D2 --> D2c[适用场景<br/>笔记/任务/事件]
...
graph TD
    ...
    G --> H[使用 Profile 模式]
    D --> I[使用 Collection 模式]
    
    H --> J{是否有嵌套列表?}
    J -->|是| K{列表项数量有限且重要?}
    K -->|是| L[作为 Profile 的 List 字段]
    K -->|否| M[拆分为 Collection]
