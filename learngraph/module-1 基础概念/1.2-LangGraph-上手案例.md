...
|---------|---------------------|------------------|---------|
| **StateGraph** | 状态图类，用于构建 LangGraph 工作流，管理节点、边和状态 | Python 类，通过 add_node() 和 add_edge() 方法定义图结构 | ⭐⭐⭐⭐⭐ |
| **MessagesState** | 内置状态类型，专门管理对话消息列表，自动累积历史 | TypedDict 类型，包含 messages 字段和 add_messages reducer | ⭐⭐⭐⭐⭐ |
| **Node** | 图中的节点函数，接收 state 参数并返回状态更新字典 | 普通 Python 函数，签名为 def node(state: State) -> dict | ⭐⭐⭐⭐⭐ |
| **START/END** | 特殊节点，标记图的入口点和出口点 | LangGraph 预定义的常量，用于 add_edge() 连接 | ⭐⭐⭐⭐⭐ |
| **MemorySaver** | 内存检查点保存器，在内存中持久化对话历史和状态 | Python 类，实现 checkpointer 接口，支持多轮对话 | ⭐⭐⭐⭐ |
| **ToolNode** | 工具执行节点，自动处理 LLM 的工具调用请求 | LangGraph 内置节点类，自动执行 tool_calls 并返回结果 | ⭐⭐⭐⭐⭐ |
| **Conditional Edge** | 条件边，根据路由函数返回值动态选择下一个节点 | add_conditional_edges() 方法定义，通过字典映射路由结果 | ⭐⭐⭐⭐⭐ |
| **Tool** | 工具函数，通过 @tool 装饰器包装，LLM 可以调用执行外部操作 | Python 函数 + @tool 装饰器，包含 docstring 作为工具描述 | ⭐⭐⭐⭐⭐ |
| **bind_tools()** | 将工具列表绑定到 LLM，使模型知道可用的工具和调用方式 | ChatModel 的方法，返回带工具能力的模型实例 | ⭐⭐⭐⭐ |
| **thread_id** | 会话标识符，用于区分不同用户或对话的 checkpoint | 配置参数，通过 config={"configurable": {"thread_id": "xxx"}} 传递 | ⭐⭐⭐⭐ |
| **ReAct Agent** | Reasoning + Acting 模式的智能体，交替进行思考和行动 | 通过循环图结构实现，包含 agent 节点和 tools 节点的循环 | ⭐⭐⭐⭐ |
| **Multi-Agent** | 多智能体系统，多个专业 Agent 协作完成复杂任务 | 通过多个节点函数实现，每个节点代表一个专业 Agent | ⭐⭐⭐⭐ |
