# 1.1 LangGraph 1.x 与其它框架对比

![LangGraph 1.0 正式发布](./images/langgraph-1.0-banner.png)
...
![Agent 循环执行流程](./images/agent-loop-diagram.png)
...
# 运行 Agent
result = weather_agent.invoke("北京今天天气怎么样？")
```

**支持结构化输出**——让 AI 返回固定格式的数据：
...
![Middleware 中间件架构](./images/middleware-hooks.png)
...
不同的 AI 模型返回的数据格式各不相同，这给开发者带来很多麻烦。LangGraph 1.0 引入了**标准内容块**，统一处理：
...
![LangChain 与 LangGraph 包结构对比](./images/package-comparison.png)
...
```bash
npm install @langchain/langchain@latest @langchain/langgraph
```
