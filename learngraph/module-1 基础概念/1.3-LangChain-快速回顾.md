...
|---------|---------------------|------------------|---------|
| **Chat Model** | 聊天模型，LangChain 对 LLM API 的封装，支持多种提供商 | Python 类(如 ChatOpenAI)，实现 invoke/stream/batch 等方法 | ⭐⭐⭐⭐⭐ |
| **Prompt Template** | 提示词模板，用于结构化管理提示词和动态变量替换 | ChatPromptTemplate 类，通过 from_messages() 创建模板 | ⭐⭐⭐⭐⭐ |
| **LCEL** | LangChain Expression Language，链式组合语法，使用管道符连接组件 | 通过重载 | 运算符实现的组件连接语法 | ⭐⭐⭐⭐⭐ |
| **Tool** | 工具，通过 @tool 装饰器定义，扩展 LLM 的能力 | Python 函数 + 装饰器，包含描述、参数 schema 和执行逻辑 | ⭐⭐⭐⭐⭐ |
| **Output Parser** | 输出解析器，将 LLM 的文本输出转换为结构化数据 | StrOutputParser、JsonOutputParser、PydanticOutputParser 等类 | ⭐⭐⭐⭐ |
| **Message** | 消息对象，表示对话中的不同角色和内容 | HumanMessage、AIMessage、SystemMessage、ToolMessage 等类 | ⭐⭐⭐⭐⭐ |
| **Runnable** | 可运行接口，所有 LangChain 组件实现的统一接口 | 抽象接口，定义 invoke/stream/batch/ainvoke 等方法 | ⭐⭐⭐⭐⭐ |
| **Retriever** | 检索器，用于从向量数据库检索相关文档 | 实现检索接口的类，通过 similarity_search() 查询 | ⭐⭐⭐⭐ |
| **RAG** | Retrieval-Augmented Generation，检索增强生成技术 | 结合检索器和 LLM 的工作流模式，先检索后生成 | ⭐⭐⭐⭐⭐ |
| **Vector Store** | 向量存储，保存文档的向量表示并支持相似度搜索 | FAISS、Chroma 等类，提供 from_documents() 和搜索方法 | ⭐⭐⭐⭐ |
| **Embedding** | 嵌入模型，将文本转换为向量表示 | OpenAIEmbeddings 等类，实现 embed_query() 和 embed_documents() | ⭐⭐⭐⭐ |
| **Pydantic** | Python 数据验证库，用于定义结构化的数据模型 | BaseModel 类，通过类型注解定义字段和验证规则 | ⭐⭐⭐⭐ |
...
graph TD
    ...
    %% 框1：LangGraph 层
    subgraph LangGraph[LangGraph 层]
        B1[StateGraph（状态图）] --> B11[节点]
        B1 --> B12[条件边/循环]
        B2[Agent 编排] --> B21[对话流]
    end
...
LangChain 支持 30+ 种模型提供商。本教程默认使用 OpenAI 模型：
...
> 💡 如需使用其他模型提供商（Anthropic、DeepSeek 等），请参考 [API Key 配置说明](./0.-1-网站使用说明.md#如何运行代码示例)
