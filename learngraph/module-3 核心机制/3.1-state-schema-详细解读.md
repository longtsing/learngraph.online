...
|---------|---------------------|------------------|---------|
| **State Schema** | LangGraph 中定义图数据结构和类型的模式，决定节点间如何传递和更新数据 | 可以用 TypedDict、Dataclass 或 Pydantic BaseModel 定义的数据结构 | ⭐⭐⭐⭐⭐ |
| **TypedDict** | LangGraph 最常用的状态定义方式，提供类型提示但无运行时验证 | Python 标准库类型，为字典添加类型注解，访问通过 `state["key"]` | ⭐⭐⭐⭐⭐ |
| **Dataclass** | LangGraph 状态定义的第二种方式，支持默认值和方法 | Python 3.7+ 的 `@dataclass` 装饰器，访问通过 `state.key` | ⭐⭐⭐⭐ |
| **Pydantic BaseModel** | LangGraph 状态定义的第三种方式，提供运行时验证和复杂校验 | 第三方库 Pydantic 的核心类，自动进行数据验证和类型转换 | ⭐⭐⭐⭐ |
| **Channel** | LangGraph 中 State 的每个字段都是一个独立通道，节点可以读写任何通道 | 状态字典中的键值对，多个节点可以独立更新不同的通道 | ⭐⭐⭐⭐⭐ |
| **Literal** | 限定字段值只能是指定的字面量之一，提供类型安全 | Python `typing` 模块的类型，如 `Literal["happy", "sad"]` | ⭐⭐⭐⭐ |
| **Annotated** | 为类型添加元数据的 Python 类型提示工具，LangGraph 用于指定 Reducer | Python `typing` 的泛型类型，格式为 `Annotated[类型, 元数据]` | ⭐⭐⭐⭐⭐ |
| **StateGraph** | LangGraph 的核心图类，接受 State Schema 作为参数来创建状态化的图 | 构造时传入状态类：`StateGraph(MyState)` | ⭐⭐⭐⭐⭐ |
| **field_validator** | Pydantic 装饰器，用于为 BaseModel 字段添加自定义验证逻辑 | 装饰器函数，在字段赋值时自动执行验证，失败抛出 ValidationError | ⭐⭐⭐ |
| **partial update** | LangGraph 节点可以只返回部分字段，未返回的字段保持不变 | 节点返回字典会与现有状态合并，而不是完全覆盖 | ⭐⭐⭐⭐⭐ |
