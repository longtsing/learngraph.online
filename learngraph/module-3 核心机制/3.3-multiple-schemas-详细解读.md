...
|---------|---------------------|------------------|---------|
| **Multiple Schemas** | LangGraph 允许在同一个图中使用多个不同的状态结构定义 | 通过类型注解指定节点输入输出类型，实现状态的分层和隔离 | ⭐⭐⭐⭐⭐ |
| **Private State** | 节点间传递但不对外暴露的私有状态数据 | 通过不同的 TypedDict 定义，内部使用但不出现在最终输出中 | ⭐⭐⭐⭐⭐ |
| **Input Schema** | 限定用户输入字段的状态模式，提高安全性 | StateGraph 参数 `input_schema` 指定，只接受定义的字段 | ⭐⭐⭐⭐ |
| **Output Schema** | 限定返回给用户字段的状态模式，过滤内部数据 | StateGraph 参数 `output_schema` 指定，只输出定义的字段 | ⭐⭐⭐⭐ |
| **OverallState** | 图内部使用的完整状态，包含所有字段（公开+私有） | TypedDict 定义，是 InputState 和 OutputState 的超集 | ⭐⭐⭐⭐⭐ |
| **状态过滤** | LangGraph 根据类型注解自动过滤节点的输入输出状态 | 节点只接收和返回其类型注解声明的字段，其他字段被过滤 | ⭐⭐⭐⭐⭐ |
| **类型注解** | Python 函数签名中的类型提示，LangGraph 用于确定状态可见范围 | 格式 `def node(state: InputType) -> OutputType`，指定输入输出类型 | ⭐⭐⭐⭐⭐ |
| **状态分层** | 将状态划分为输入层、内部层、输出层的设计模式 | InputState ⊆ OverallState ⊇ OutputState 的包含关系 | ⭐⭐⭐⭐ |
| **NotRequired** | TypedDict 中标记可选字段的类型注解 | Python typing 模块类型，如 `optional: NotRequired[int]` | ⭐⭐⭐ |
| **状态继承** | TypedDict 可以继承其他 TypedDict 来复用字段定义 | `class ExtendedState(BaseState)` 继承父类所有字段并添加新字段 | ⭐⭐⭐ |
...
![Flow Diagram](images/multiple-schemas-output-4-0.png)
...
![Flow Diagram](images/multiple-schemas-output-8-0.png)
...
![Flow Diagram](images/multiple-schemas-output-12-0.png)
...
以下是一个完整的 Multiple Schemas 示例，展示私有状态和输入/输出模式的使用：
...
# 内部状态：包含所有字段（输入 + 输出 + 私有）
class OverallState(TypedDict):
    question: str      # 来自输入
    answer: str        # 输出给用户
    notes: str         # 私有字段：内部思考过程
    api_calls: int     # 私有字段：API 调用次数
...
def thinking_node(state: InputState):
    """
    思考节点：处理用户问题
    - 输入：只能访问 InputState 中的字段
    - 输出：可以写入任何 OverallState 中的字段
    """
    print(f"---思考节点---")
    print(f"收到问题: {state['question']}")

    # 模拟思考过程
    notes = f"用户问了 '{state['question']}'，我需要分析这个问题..."

    return {
        "answer": f"让我来回答关于 {state['question']} 的问题",
        "notes": notes,
        "api_calls": 1
    }
...
def answer_node(state: OverallState) -> OutputState:
    """
    回答节点：生成最终答案
    - 输入：可以访问完整的 OverallState
    - 输出：只返回 OutputState 定义的字段
    """
    print(f"---回答节点---")
    print(f"内部笔记: {state.get('notes', 'N/A')}")  # 可以访问私有字段
    print(f"API 调用次数: {state.get('api_calls', 0)}")

    # 基于所有信息生成最终答案
    final_answer = f"{state['answer']}。根据我的分析，这是一个很好的问题！"

    return {"answer": final_answer}

# 3. 构建图 - 指定输入和输出模式
builder = StateGraph(
    OverallState,              # 图内部使用完整状态
    input_schema=InputState,    # 限定用户输入
    output_schema=OutputState   # 限定用户输出
)
...
# 7. 查看结果
print("\n=== 最终结果 ===")
print(f"返回给用户的结果: {result}")

# 注意：notes 和 api_calls 被过滤掉了！
print("\n=== Multiple Schemas 演示成功！===")
print("私有字段 'notes' 和 'api_calls' 不会出现在最终输出中")
print("用户只能看到 OutputState 定义的字段")
```
