...
![Thread and Checkpoints Visualization](https://cdn.prod.website-files.com/65b8cd72835ceeacd4449a53/66dbadf3b379c2ee621adfd1_chatbot-summarization1.png)
...
以下是一个完整的对话摘要示例，展示如何使用 LLM 生成摘要来压缩对话历史：
...
# 2. 定义状态 - 继承 MessagesState 并添加 summary 字段
class State(MessagesState):
    summary: str

# 3. 对话节点：使用摘要作为上下文
def call_model(state: State):
    """调用 LLM 生成回复，如果有摘要则包含在系统消息中"""
    summary = state.get("summary", "")
...
# 4. 摘要节点：生成/更新摘要并删除旧消息
def summarize_conversation(state: State):
    """生成对话摘要并删除旧消息，只保留最新 2 条"""
    summary = state.get("summary", "")
    ...
    if summary:
        ...
        summary_message = "请为上面的对话创建一个简洁的摘要:"
...
for i, msg in enumerate(conversations, 1):
    ...
    output = graph.invoke({"messages": [HumanMessage(content=msg)]}, config)
    ...
    print(f"[当前消息数: {msg_count}, 有摘要: {'是' if summary else '否'}]")

# 10. 查看最终状态
print("\n=== 最终状态 ===")
final_state = graph.get_state(config)
print(f"消息数: {len(final_state.values.get('messages', []))}")
print(f"摘要: {final_state.values.get('summary', '无')[:200]}...")

print("\n=== 对话摘要演示成功！===")
print("对话超过 6 条后自动生成摘要，并删除旧消息只保留最新 2 条")
```
