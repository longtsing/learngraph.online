...
|---------|---------------------|------------------|---------|
| **External Memory** | LangGraph 使用外部数据库存储状态，实现跨会话持久化 | 通过 Checkpointer 连接数据库，状态保存到磁盘而非内存 | ⭐⭐⭐⭐⭐ |
| **SqliteSaver** | LangGraph 提供的 SQLite 数据库 Checkpointer 实现 | `from langgraph.checkpoint.sqlite import SqliteSaver` | ⭐⭐⭐⭐⭐ |
| **Checkpointer** | LangGraph 状态持久化的抽象接口，负责保存和加载状态 | 可用 MemorySaver（内存）、SqliteSaver（SQLite）、PostgresSaver | ⭐⭐⭐⭐⭐ |
| **SQLite** | 轻量级嵌入式关系数据库，无需独立服务器，数据存储在单个文件 | Python 内置 `sqlite3` 模块支持，`conn = sqlite3.connect("db.db")` | ⭐⭐⭐⭐⭐ |
| **Thread** | LangGraph 用于隔离不同对话会话的线程标识符 | 配置中的 `thread_id`，不同 ID 的对话状态完全独立 | ⭐⭐⭐⭐⭐ |
| **StateSnapshot** | 图在特定检查点的完整状态快照，包含所有字段和元数据 | `graph.get_state(config)` 返回，包含 values、config、metadata 等 | ⭐⭐⭐⭐ |
| **check_same_thread** | SQLite 连接参数，False 允许多线程访问同一连接 | `sqlite3.connect(db, check_same_thread=False)` LangGraph 需要此设置 | ⭐⭐⭐ |
| **getpass** | Python 模块，用于安全输入密码和 API Key，不显示输入内容 | `import getpass; key = getpass.getpass("API Key: ")` | ⭐⭐⭐ |
| **持久化** | 数据存储到磁盘，程序重启后仍可恢复，与内存存储相对 | SQLite 文件持久存储，MemorySaver 重启后丢失 | ⭐⭐⭐⭐⭐ |
| **checkpoint_id** | 每个状态快照的唯一标识符，用于精确恢复到某个历史状态 | StateSnapshot.config 中的 checkpoint_id 字段 | ⭐⭐⭐ |
...
def call_model(state: State):
    ...
    if summary:
        system_message = f"对话历史摘要: {summary}"
        messages = [SystemMessage(content=system_message)] + state["messages"]
    else:
        messages = state["messages"]
...
def summarize_conversation(state: State):
    ...
    if summary:
        summary_message = f"现有摘要: {summary}\n\n请扩展此摘要以包含新的对话内容:"
    else:
        summary_message = "请为这段对话创建一个简洁的摘要:"
...
config_alice = {"configurable": {"thread_id": "alice"}}
print("\n--- Alice 的对话 ---")
result = graph.invoke({"messages": [HumanMessage("你好！我是 Alice")]}, config_alice)
print(f"Alice: 你好！我是 Alice")
print(f"AI: {result['messages'][-1].content}")

# 线程 2：用户 Bob
config_bob = {"configurable": {"thread_id": "bob"}}
print("\n--- Bob 的对话 ---")
result = graph.invoke({"messages": [HumanMessage("你好！我是 Bob")]}, config_bob)
print(f"Bob: 你好！我是 Bob")
print(f"AI: {result['messages'][-1].content}")

# Alice 继续对话 - 验证记忆
print("\n--- Alice 继续对话（验证记忆）---")
result = graph.invoke({"messages": [HumanMessage("你还记得我的名字吗？")]}, config_alice)
print(f"Alice: 你还记得我的名字吗？")
print(f"AI: {result['messages'][-1].content}")

# 10. 查看 Alice 的状态
print("\n=== 查看 Alice 的状态 ===")
alice_state = graph.get_state(config_alice)
print(f"消息数: {len(alice_state.values.get('messages', []))}")
print(f"有摘要: {'是' if alice_state.values.get('summary') else '否'}")

# 11. 查看 Bob 的状态
print("\n=== 查看 Bob 的状态 ===")
bob_state = graph.get_state(config_bob)
print(f"消息数: {len(bob_state.values.get('messages', []))}")
print(f"有摘要: {'是' if bob_state.values.get('summary') else '否'}")

print("\n=== 外部记忆演示成功！===")
print("不同 thread_id 的对话状态完全独立")
print("使用 SqliteSaver 可以实现程序重启后状态恢复")
```
