...
|---------|---------------------|------------------|---------|
| **RemoveMessage** | LangGraph 用于从状态中永久删除消息的特殊消息类型 | LangChain 消息类，格式 `RemoveMessage(id="msg_id")`，配合 add_messages reducer | ⭐⭐⭐⭐⭐ |
| **trim_messages** | LangChain 工具函数，基于 token 数量智能裁剪消息列表 | 函数签名 `trim_messages(messages, max_tokens, strategy, token_counter)` | ⭐⭐⭐⭐⭐ |
| **filter_messages** | 通过条件筛选消息，不修改状态，只改变传递给 LLM 的内容 | 列表切片或过滤函数，如 `messages[-5:]` 只保留最后5条 | ⭐⭐⭐⭐ |
| **MessagesState** | LangGraph 预定义状态类，包含带 add_messages reducer 的 messages 字段 | TypedDict 基类，自动处理消息追加和 RemoveMessage 删除 | ⭐⭐⭐⭐⭐ |
| **token_counter** | trim_messages 参数，用于计算消息 token 数量的模型实例 | ChatOpenAI 或其他 LLM 实例，提供 `get_num_tokens_from_messages()` 方法 | ⭐⭐⭐⭐ |
| **strategy** | trim_messages 的裁剪策略，决定保留哪些消息 | "last" 保留最后的消息，"first" 保留最开始的消息 | ⭐⭐⭐⭐ |
| **allow_partial** | trim_messages 参数，是否允许截断单条消息以达到 token 限制 | 布尔值，False 整条保留或丢弃，True 允许部分截断 | ⭐⭐⭐ |
| **滑动窗口** | 只保留最近 N 条消息的内存管理模式 | 通过列表切片 `messages[-N:]` 或定期删除旧消息实现 | ⭐⭐⭐⭐ |
| **SystemMessage** | LangChain 消息类型，表示系统指令或提示 | 通常放在消息列表开头，包含角色设定和规则 | ⭐⭐⭐⭐ |
| **消息 ID** | 每条消息的唯一标识符，用于 RemoveMessage 定位删除 | 消息对象的 `id` 属性，自动生成或手动指定 | ⭐⭐⭐⭐⭐ |
