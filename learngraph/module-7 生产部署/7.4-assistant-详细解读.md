...
![Assistants Architecture](https://cdn.prod.website-files.com/65b8cd72835ceeacd4449a53/673d50597f4e9eae9abf4869_Screenshot%202024-11-19%20at%206.57.01%E2%80%AFPM.png)
...
|---------|---------------------|------------------|---------|
| **Assistant** | 图 + 配置的组合，为同一个图创建不同的"人格"或行为变体，每个 assistant 有唯一的 ID。 | 通过 `client.assistants.create(graph_id, config)` 创建，存储在 PostgreSQL，包含配置、元数据和版本信息。 | ⭐⭐⭐⭐⭐ |
| **assistant_id** | 助手的唯一标识符（UUID），用于在运行时引用特定的 assistant。 | 替代 graph_name 使用，如 `client.runs.create(thread_id, assistant_id, input)`，简化配置管理。 | ⭐⭐⭐⭐⭐ |
| **Configurable** | 图的可配置字段定义类，继承自 Configuration，定义 user_id、todo_category 等可定制参数。 | Python 数据类，使用 Pydantic Field 定义默认值，在节点中通过 `config["configurable"]` 访问。 | ⭐⭐⭐⭐⭐ |
| **Version Control** | Assistant 的版本控制机制，每次更新 assistant 时版本号自动递增，支持追踪变化和回滚。 | `version` 字段从 1 开始，`updated_at` 时间戳更新，可通过 `assistants.get(assistant_id, version=n)` 获取特定版本。 | ⭐⭐⭐⭐ |
| **todo_category** | task_maistro 图的配置参数，定义待办类别（personal/work/family 等），用于组织和隔离不同类型任务。 | 字符串类型，在 Store namespace 中使用：`("todo", todo_category, user_id)`。 | ⭐⭐⭐⭐ |
| **task_maistro_role** | 助手的系统提示词配置，定义 AI 的行为风格、职责和沟通方式，是 assistant 个性化的核心。 | 长字符串，包含角色定义、任务重点、沟通风格等，在 LLM 调用时作为 system message 使用。 | ⭐⭐⭐⭐⭐ |
| **Metadata** | Assistant 的元数据字典，用于存储名称、描述、变更日志等自定义信息。 | Python 字典类型，可包含任意键值对，方便管理和检索 assistants。 | ⭐⭐⭐ |
| **A/B Testing** | 创建同一图的多个 assistant 变体，分配给不同用户组，比较不同配置的效果。 | 通过创建多个 assistants（如 v1 简短回复、v2 详细回复），根据用户分组选择不同 assistant_id。 | ⭐⭐⭐⭐ |
| **assistants.search()** | 搜索和列出所有 assistants 的 API 方法，支持按 graph_id 过滤和分页。 | 异步方法，返回 assistant 列表，支持 `graph_id`、`limit`、`offset` 参数。 | ⭐⭐⭐⭐ |
| **assistants.update()** | 更新现有 assistant 的配置，创建新版本但保持相同的 assistant_id。 | `await client.assistants.update(assistant_id, config=new_config)`，版本号递增，updated_at 更新。 | ⭐⭐⭐⭐ |
| **assistants.delete()** | 删除指定的 assistant，不可恢复，通常用于清理测试或过期的 assistants。 | `await client.assistants.delete(assistant_id)`，删除后无法通过该 ID 创建运行。 | ⭐⭐⭐ |
| **Dynamic Selection** | 根据用户请求或上下文动态选择合适的 assistant，实现智能路由。 | 分析用户输入关键词（如 work/family），从 assistants 列表中选择匹配的 assistant_id。 | ⭐⭐⭐⭐ |
